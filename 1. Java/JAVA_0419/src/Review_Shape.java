...
class Circle extends Shape{ // 추상 클래스인 Shape 상속 -> Shape의 추상 메소드 재정의(오버라이딩)
	...
	// Circle 생성자 구현
	public Circle(int r) {
		this.radius = r; //this를 이용해 위치를 정확하게 지정하는게 좋다
	}
	...
	public void Print() {
		...
		System.out.println("반지름: " + this.radius); // this.radius :: Print함수를 호출한 객체의 반지름. this를 안해도 상관없다.
	}
...
class Triangle extends Shape{
	...
	// Triangle 생성자 구현
	public Triangle(int w, int h, boolean tf) { // 매개변수와 멤버변수의 이름이 같다면 헷갈리지 않도록 this를 써줘야된다.
		this.w = w;
		this.h = h;
		this.tf = tf;
	}
